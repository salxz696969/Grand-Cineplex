...
export default function Movies() {
    ...
    if (addingMovie) {
        return <AddMovie onBack={handleBackToMovies} />;
    }
            ...
            <div className="flex flex-col gap-4 w-full bg-gray-950 min-h-screen overflow-y-auto overflow-x-hidden">
                ...
                <div className="grid w-full gap-6 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 mt-6">
                    {[...Array(6)].map((_, i) => (
                        <div
                            key={i}
                            className="overflow-hidden shadow-lg flex flex-col bg-gray-950 border border-gray-800 rounded-xl"
                        >
    ...
    if (editMovie) {
        return <EditMovie onBack={() => setEditMovie(null)} movie={editMovie} />;
    }
        ...
        <div className="flex flex-col gap-4  w-full bg-gray-950 min-h-screen overflow-y-auto overflow-x-hidden">
            ...
            <div className="flex flex-row items-center justify-between  mt-4 gap-4">
                <div className="flex flex-row items-center justify-start gap-4">
                    <button
                        className={`text-white py-2 px-1 flex items-center border-b ${activeTab === "recent" ? "border-blue-600 font-bold" : "border-transparent"
                            }`}
                ...
                <input
                    ...
                    className="w-full max-w-md rounded-full border border-gray-800 bg-gray-900 px-5 py-2 text-white "
                />
            </div>
            <div className="grid w-full gap-6 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 mt-6">
                {activeTab === "recent" ? (
                    ...
                    allMovies.map((movie) => (
                        <MovieCard key={movie.id} movie={movie} onEdit={() => setEditMovie(movie)} />
                    ))
                ) : (
