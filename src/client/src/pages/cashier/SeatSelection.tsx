                            ...
                            <div className="flex gap-1">
                                {seats.filter(seat => seat.row === row).map((seat) => {
                                    ...
                                    return (
                                        <button
                                            key={seat.id}
                                            className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-medium transition-all duration-200 transform hover:scale-110 ${getSeatStyle(seat, isSelected)}`}
                        ...
                        <div className="space-y-3">
                            <div className="flex flex-wrap gap-2">
                                {selectedSeats.sort().map(seatId => {
                                    const seat = seats.find(s => s.id === seatId);
                                    return (
                                        <div key={seatId} className="bg-blue-600/20 border border-blue-500/30 px-3 py-1 rounded-full text-blue-300 text-sm">
                                            {seatId} - ${seat?.price}
                                        </div>
                                    );
                                })}
