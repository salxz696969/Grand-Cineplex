...
export default function Bookings() {
    ...
    const filteredBookings = bookings.filter(booking => {
        const matchesTab = activeTab === "today" ? booking.date === today : true;
        const matchesSearch = booking.customerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
            booking.movieTitle.toLowerCase().includes(searchTerm.toLowerCase()) ||
            booking.customerEmail.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesStatus = selectedStatus === "all" || booking.status === selectedStatus;
        const matchesMethod = selectedMethod === "all" || booking.bookingMethod === selectedMethod;
        ...
        <div className="flex flex-col gap-6 p-4 w-full">
            ...
            <div className="flex flex-row items-center justify-between">
                <div className="flex flex-col">
                    <h2 className="text-2xl font-bold tracking-tight text-white">Bookings</h2>
                    <p className="text-slate-400">Manage customer bookings and tickets.</p>
                </div>
            </div>
